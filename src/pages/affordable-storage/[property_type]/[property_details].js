...
const PropertDetails = ({ data })=>{
  ...
  const handleSubmit = async (e) => {
    ...
    if (response.success) {
      ...
      router.push(`/affordable-storage/booking-space/${urlArr[urlArr.length-1]}`);
    } else {
...
export async function getServerSideProps(context) {
    ...
    const isProduction = host && (host.includes("stage.xtendedspace.com") || host.includes("localhost") || host.includes("main.d2kyc"));
