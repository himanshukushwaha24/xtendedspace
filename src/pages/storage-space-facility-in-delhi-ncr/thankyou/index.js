...
const Thankyou = () => {
  ...
  useEffect(() => {
    const storedata =
      localStorageManager.getValue("landing_page_data") &&
      JSON.parse(localStorageManager.getValue("landing_page_data"));
    ...
    const saveData = async (storedData) => {
      try {
        ...
        if (error.errors) {
          ...
          console.error("Error calling SaveDelhiLandingData API:", error);
        }
      }
    };
  ...
  return (
    <div className="packerWrapper">
      <section className="thanks">
        ...
        <a className="thanks-btn" href="/storage-space-facility-in-delhi-ncr/">
          Back home
        </a>
